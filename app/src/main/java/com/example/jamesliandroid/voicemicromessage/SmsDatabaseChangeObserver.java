...
public class SmsDatabaseChangeObserver  extends ContentObserver {
    ...
    public SmsDatabaseChangeObserver(ContentResolver resolver, Handler handler) {
        super(handler);
        mResolver = resolver;
    }

    public SmsDatabaseChangeObserver(ContentResolver resolver, Handler handler, String smsNo) {
        super(handler);
        mResolver = resolver;
        this.smsNo = smsNo;
    }
    ...
    private void onReceiveSms() {
        ...
        try {
            ...
            if (cursor != null) {
                ...
                if (TextUtils.isEmpty(strAddress) || TextUtils.isEmpty(strbody) || !smsNo.equals(strAddress)) {
                    return;
                }
                // 得到短信号码和内容之后进行相关处理
                handlerVoiceMessageInterface.handleVoiceMessage(strbody);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    private HandlerVoiceMessageInterface handlerVoiceMessageInterface;

    public void setHandlerVoiceMessageInterface(HandlerVoiceMessageInterface handlerVoiceMessageInterface) {
        this.handlerVoiceMessageInterface = handlerVoiceMessageInterface;
    }

    public interface HandlerVoiceMessageInterface {
        void handleVoiceMessage(String content);
    }
}